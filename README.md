...
- Ускоренная транскрибация (2x скорость)
...
3. **Транскрибация**: Нажмите "Начать транскрибацию" для обработки
   - Аудио обрабатывается с ускорением 2x для быстрой обработки
   ...
   - Транскрибация займет примерно половину длительности аудио
   - Даже если речь не распознается полностью, создается результат
...
- Если ошибка постоянно повторяется - перезагрузите страницу
...
3. Убедитесь, что используете **Chrome** (не Firefox/Safari)
4. Попробуйте перезагрузить страницу и начать заново
5. Проверьте интернет-соединение
6. Даже если речь не распознана, должна появиться заглушка "[Аудио обработано, но речь не распознана...]"

### Приложение лагает
- Не открывайте несколько транскрибаций одновременно
- Закройте другие вкладки браузера для освобождения памяти
- Используйте более мощное устройство для больших файлов
- Разделите длинные аудио (>30 мин) на части
