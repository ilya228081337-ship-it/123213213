    ...
    return new Promise((resolve, reject) => {
      ...
      this.recognition.onerror = (event: any) => {
        ...
        if (event.error === 'no-speech') {
          ...
          if (noSpeechCount < 20 && shouldContinue) {
            recognitionRestartTimeout = window.setTimeout(() => {
              startRecognition();
            }, 100);
          } else if (!shouldContinue || this.audioContext!.currentTime >= duration * source.playbackRate.value) {
      ...
      this.recognition.onend = () => {
        ...
        if (shouldContinue && this.audioContext!.currentTime < duration * source.playbackRate.value - 0.5) {
          recognitionRestartTimeout = window.setTimeout(() => {
            startRecognition();
          }, 50);
        } else {
